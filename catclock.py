...
for i in range(6):
    im = image.imread(r'F:\pycharmWork\GUI闹钟\{}.png'.format(i % 6))
...
def update(i):
    ...
    anchor.set_data([0, anchor_ratio * math.cos(theta[i]) * (r_out - 2 * r_in)],
                    [0, anchor_ratio * math.sin(theta[i]) * (r_out - 2 * r_in)])
